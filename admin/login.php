...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($admin && password_verify($password, $admin['password'])) {
            ...
            $session_token = bin2hex(random_bytes(32)); // 64-character random token

            // Store the session token in the database
            $stmt = $pdo->prepare("UPDATE admin SET session_token = ? WHERE id = ?");
            $stmt->execute([$session_token, $admin['id']]);
        ...
        <div class="right">
            ...
            <form action="" method="POST">
                ...
                <div class="password">
                    ...
                    <img src="../assets/image/eye-closed-svgrepo-com.svg" alt="hide" onclick="pass()" class="pass-icon" id="pass-icon">
                </div>
