...
try {
    ...
    if ($is_admin) {
        $stmt = $pdo->prepare("
            SELECT dl.id, dl.student_id, 
                   DATE(dl.time_in) AS duty_date, 
                   DATE_FORMAT(dl.time_in, '%Y-%m-%d %H:%i') AS time_in, 
                   DATE_FORMAT(dl.time_out, '%Y-%m-%d %H:%i') AS time_out, 
        ...
        $stmt = $pdo->prepare("
            SELECT DATE(dl.time_in) AS duty_date, 
                   DATE_FORMAT(dl.time_in, '%Y-%m-%d %H:%i') AS time_in, 
                   DATE_FORMAT(dl.time_out, '%Y-%m-%d %H:%i') AS time_out, 
                   dl.total_hours, dl.status 
            FROM duty_logs dl
            WHERE student_id = ? 
            ORDER BY dl.time_in DESC
        ");
